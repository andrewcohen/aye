...
// CLI verbatim, which is why the window has nothing to do but send it.
//
//   /new · /mcp   the window acts, nothing is sent          `mine: true`
//   /bro · …      delivered as ordinary text                `mine: false`
//
// The rule for intercepting is narrow on purpose: an exact match on the whole
// draft, and only against the window's own two. A person who types
...
export interface Command {
  ...
  /**
   * Whether the window runs it, rather than the agent.
   *
   ...
   * which is the right way round: the window's two are documented in this
...
export const matching = (
  ...
  // The window's own first, then the agent's by name. Not because ours matter
...
export const commandOf = (draft: string): Command | undefined => {
  ...
  // The window's own only. An agent command that reached here would be run by
  // the window and never delivered, which for a skill is the difference
 ...
 *
 * The name and no trailing space for the window's own: a space would make
