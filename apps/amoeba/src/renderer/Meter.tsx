...
export function Meter({ chrome }: { readonly chrome: Chrome }) {
  ...
  //
  // A ref and not state, because nothing renders it — it exists to be read when
  // the copy button is pressed. As state it would re-render the column four
  // times a second to update a number nobody is looking at.
  const peak = useRef<Rate>(emptyRate);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      setRate(sample);
      peak.current = peakOf(peak.current, sample);
      setReading(next);
      previous = next;
    }, SAMPLE_MS);
      ...
      <div style={{ display: "flex", gap: "0.35rem", marginTop: "1rem" }}>
        <button
          ...
          onClick={() => {
            ...
            void (async () => {
              await copyReading(reading, rate, peak.current);
              setCopied(true);
